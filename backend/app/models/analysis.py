...
class SquareControl(BaseModel):
    ...
    white_control: List[List[int]] = Field(default_factory=lambda: [[0 for _ in range(8)] for _ in range(8)])
    black_control: List[List[int]] = Field(default_factory=lambda: [[0 for _ in range(8)] for _ in range(8)])
    white_control_material: List[List[int]] = Field(default_factory=lambda: [[0 for _ in range(8)] for _ in range(8)])
    black_control_material: List[List[int]] = Field(default_factory=lambda: [[0 for _ in range(8)] for _ in range(8)])
...
class PositionAnalysis(BaseModel):
    ...
    critical_squares: List[Tuple[str, str]] = Field(default_factory=list)  # [(square, description)]
...
class GameAnalysisResult(BaseModel):
    ...
    player_weaknesses: Dict[str, List[int]] = Field(default_factory=lambda: {
        "tactical": [],
        "positional": [],
        "opening": [],
        "endgame": []
    })
    critical_positions: List[int] = Field(default_factory=list)  # List of move numbers where game shifted significantly